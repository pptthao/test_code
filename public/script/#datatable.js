        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                if (res != 0) {
                    ...
                    // $("#example2").DataTable().off();
                    // for (var i = 0; i < res.length; i++) {
                    //     var table = '';
                    //     table += '<tr class="test" role="row">';
                    //     i++;
                    //     table += '<td>' + i + '</td>';
                    //     i--;
                    //     table += '<td>' + res[i]['masv'] + '</td>';
                    //     table += '<td>' + res[i]['tensv'] + '</td>';
                    //     table += '<td><button type="button" value="' + res[i]['id'] + '" name="sua" class="btn btn-primary btnSua '+ res[i]['id'] + '" data-toggle="modal" data-target="#modaledit" id="' + res[i]['id'] + '">Sửa</button>&emsp;<button type="button" value="' + res[i]['id'] + '" name="xoa" class="btn btn-danger btnXoa" id="" onclick= "xacnhanxoa()">Xóa</button></td></td>';
                    //     table += '</tr>';
                    //     $(table).appendTo('#dsmon');
                    // }
                    // $("#example2").DataTable().ajax.reload();
                    
                        var table = '';
                        table += '<tr class="test" role="row">';
                        table += '<td>' + res['count'] + '</td>';
                        table += '<td>' + masv + '</td>';
                        table += '<td>' + tensv + '</td>';
                        table += '<td><button type="button" value="' + res['id'] + '" name="sua" class="btn btn-primary btnSua '+ res['id'] + '" data-toggle="modal" data-target="#modaledit" id="' + res['id'] + '">Sửa</button>&emsp;<button type="button" value="' + res['id'] + '" name="xoa" class="btn btn-danger btnXoa" id="" onclick= "xacnhanxoa()">Xóa</button></td></td>';
                        table += '</tr>';
                        $(table).appendTo('#dsmon');
